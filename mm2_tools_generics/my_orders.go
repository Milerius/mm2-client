...
func renderTableMakerOrders(withFees bool, makerOrders map[string]mm2_data_structure.MakerOrderContent) {
	...
	for _, cur := range makerOrders {
		curData := []string{cur.Base, cur.MinBaseVol, cur.AvailableAmount, "", helpers.BigFloatMultiply(cur.AvailableAmount, cur.Price, 8), cur.Rel, helpers.BigFloatMultiply(cur.Price, "1", 8), cur.Uuid}
		data[i] = curData
		wg.Add(1)
